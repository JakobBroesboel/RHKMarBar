...
$tag = 'marbar2017'; // tag for which ou want images 
$results_array = scrape_insta_hash($tag);
$limit = 3; // provide the limit thats important because one page only give some images then load more have to be clicked
$image_array= array(); // array to store images.
	for ($i=0; $i < $limit; $i++) { 
		$latest_array = $results_array['entry_data']['TagPage'][0]['tag']['media']['nodes'][$i];
	 	$image_data  = '<img class="instagramSlides" src="'.$latest_array['thumbnail_src'].'" style="width:400px;height:400px">'; 
	 	// thumbnail and same sizes 
